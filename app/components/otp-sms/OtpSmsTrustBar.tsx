...
export default function OtpSmsTrustBar() {
  const openModal = useModalStore((s) => s.openModal);

  return (
    <section aria-label="OTP SMS trust stats strip" className="bg-white border-y border-(--border-subtle)">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 py-6">
        <h2 className="sr-only text-2xl sm:text-[28px] md:text-4xl font-bold leading-[1.15] tracking-[-0.02em] [font-family:'Syne',sans-serif] text-foreground">
          Trusted OTP SMS provider for thousands of businesses across India
        </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 flex-1 divide-y sm:divide-y-0 sm:divide-x divide-(--border-subtle) w-full">
            {stats.map(({ icon: Icon, value, label }) => (
              ...
              >
                ...
                <span className="text-3xl font-extrabold text-(--ink-1) leading-tight">
                  {value}
                </span>
